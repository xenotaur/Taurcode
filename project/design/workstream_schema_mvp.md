...
This document defines the documentation-level MVP schema for Taurcode workstream artifacts. It
adopts the LRH workstream schema so that `lrh validate`, `lrh snapshot project`, and
`lrh workstreams organize` operate over Taurcode's `project/workstreams/` the same way they do in
other LRH-structured repositories.
...
project-control convention only; it does not implement runtime loaders, validation logic, or CLI
behavior in this repository — those live in the installed `lrh` toolchain.
...
`lrh validate` reports drift when metadata and directory placement disagree; `lrh workstreams
organize` reconciles that drift based on metadata. This mirrors the existing work-item policy that
metadata is the source of truth and bucket placement is derived.
...
automated execution, automatic stage advancement, or runtime enforcement.
...
Status is coarse lifecycle state; stage is finer process position.
...
Workstream, and Work Item relationships. The planning-tree language is an internal-ready model, not
a new user-facing directory taxonomy.

## Non-goals

This schema MVP is a project-control convention. It does not add Taurcode runtime loaders, typed
models, validation rules, CLI behavior, execution backends, or agent orchestration. Those
capabilities, if needed, should land through later focused work items.
