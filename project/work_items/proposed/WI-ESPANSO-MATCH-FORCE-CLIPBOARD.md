...
4. `src/taurcode/semantic_normalize.py` — capture `targets.espanso.force_clipboard` explicitly on `NormalizedPrompt` in both `normalize_canonical_prompts` and `normalize_espanso_package`; compare it in `_compare_canonical_prompt_fields`; add a roundtrip regression fixture for a dropped/corrupted field.
