# Taurcode STYLE GUIDE

This document defines the coding, packaging, and contribution conventions for the Taurcode project.
...
## TL;DR — The 10 Rules of Taurcode
...
1. Project-specific Taurcode conventions in this document
...
In general, code and changes in Taurcode should favor:
...
Taurcode should use a clearly documented Python version and a reproducible development setup.
...
Taurcode prefers importing modules and submodules rather than importing individual members into the local namespace, inspired by the [Google Python Style Guide Imports section](https://google.github.io/styleguide/pyguide.html#s2.2-imports).
...
from taurcode.utils import file_ops
```
...
```python
from taurcode.cli import run_cli
from taurcode.utils.file_ops import atomic_write
```
...
Taurcode targets Python 3.11. Type annotations should follow modern Python typing practices.
...
`src/taurcode/<package>/<module>.py`, prefer the matching test file:
...
- `src/taurcode/cli/taurcode.py` -> `tests/cli_tests/taurcode_test.py`
- `src/taurcode/config/config.py` -> `tests/config_tests/config_test.py`
- `src/taurcode/project/project.py` -> `tests/project_tests/project_test.py`

Avoid flattening package-specific tests into names such as
`cli_taurcode_test.py` or `config_config_test.py` at the root of `tests/`.
...
Taurcode prefers predictable, automatable tests that can be run from the command line through the project’s scripts.
...
Taurcode uses Ruff for linting.
...
Taurcode uses Black for formatting.
...
Taurcode should maintain a clear package structure and a reproducible installation path for development.
...
3. Do tests pass?
4. Does it follow Taurcode conventions?
...
Tracking work in prompt-heavy workflows can become confusing, so Taurcode provides lightweight guidance and tooling to label meaningful prompts with unique IDs and record executions.
...
Do not let prompt bookkeeping block small obvious fixes or exploratory work. Taurcode prefers traceability, but process should remain lightweight.
