...
def _read_prompt_file(prompt_file: Path) -> ExistingPrompt:
    ...
    # Performance optimization: Uses str.find() and slicing instead of split("\n")
    # to avoid allocating many intermediate string objects for large prompts.
    closing_index = normalized.find("\n---\n", 3)
    if closing_index != -1:
        metadata_text = normalized[4:closing_index]
        body = normalized[closing_index + 5 :]
    elif normalized.endswith("\n---"):
        metadata_text = normalized[4 : len(normalized) - 4]
        body = ""
    else:
