...
def _match_existing_prompt(
    match: dict,
    existing_by_trigger: dict[str, list[ExistingPrompt]],
    existing_by_stem: dict[str, list[ExistingPrompt]],
) -> ExistingPrompt | None:
    trigger = str(match["trigger"])
    trigger_matches = existing_by_trigger.get(trigger, [])
    ...
    stem_matches = existing_by_stem.get(prompt_id, [])
...
def _merge_simple_matches(
    ...
    # Pre-compute indexes for faster O(1) lookups instead of O(N) list traversals
    existing_by_trigger: dict[str, list[ExistingPrompt]] = {}
    existing_by_stem: dict[str, list[ExistingPrompt]] = {}
    for prompt in existing_prompts:
        keyword = str(prompt.metadata.get("keyword", ""))
        existing_by_trigger.setdefault(keyword, []).append(prompt)
        existing_by_stem.setdefault(prompt.path.stem, []).append(prompt)
    ...
    for match, _raw_block in entries:
        ...
        existing = _match_existing_prompt(match, existing_by_trigger, existing_by_stem)
        if existing is None:
