...
`/lrh-implement` does before its own Step 6. This keeps a primary record in
place from the start, so `/lrh-review-response` running afterward (finding
nothing to do, harmlessly) and `/lrh-closeout` later never need to
